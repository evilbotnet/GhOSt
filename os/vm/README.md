# GhOSt dev VM (UTM on Apple Silicon)
...
2. Create a VM from the UTM gallery's **Debian 13 (ARM64)** image, or install
   ...
   provisioning creates the separate `ghost` kiosk user.
...
```sh
GHOST_VM=admin@192.168.64.X ./scripts/deploy-vm.sh
```

This cross-compiles ghostd, builds the shell, rsyncs everything in, runs
`provision.sh`, and restarts greetd. The VM console should land in the GhOSt
...
- [ ] **risk check first**: `wlrctl toplevel list` over SSH — confirm the
      ...
      `os/overlay/usr/share/ghost/labwc/rc.xml` (fallback documented in plan)
...
- [ ] `systemctl --user -M ghost@ status ghost-daemon` is active
